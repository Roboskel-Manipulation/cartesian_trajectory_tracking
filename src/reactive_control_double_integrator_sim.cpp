...
void human_motion_callback(const geometry_msgs::PointStampedConstPtr human_msg){
	...
	if (count > 1){
		...
		if (sqrt(pow(des_x, 2)+pow(des_y, 2)) < self_col_dis and des_z < z_dis){
			...
			human_position->header.stamp = ros::Time::now();
...
int main(int argc, char** argv){
	...
	error_pub = n.advertise<geometry_msgs::TwistStamped>("com_vel_topic", 100);
	real_vel_sim_pub = n.advertise<geometry_msgs::TwistStamped>("real_vel_topic", 100);
